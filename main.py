...
class SimpleRobotControl:
    ...
    def asserv(self, m=None):
        ...
        x_speed_error = m.x_goal - m.x
        y_speed_error = m.y_goal - m.y

        if not (x_speed_error == 0 and y_speed_error == 0):
            m.theta_goal = math.atan2(y_speed_error, m.x_goal + m.x)

        theta_angle_error = self.angle_diff(m.theta_goal, m.theta)

        action = TURN_P * theta_angle_error
    ...
    def angle_diff(self, a, b):
        ...
        """
        #smallest angles between the two angles parameters
